...
// 4. Definisco la BASE_URL per l'endpoint principale delle offerte.
...
async function handleResponse(response) {
    if (!response.ok) {
        ...
        if (errorBody.error || errorBody.details) {
            // Gestisco l'errore di validazione 400 in modo specifico per Zod
            if (response.status === 400) {
                // Lancio un errore che contiene i dettagli per l'interfaccia utente
                const validationError = new Error("Errore di validazione dei dati inviati.");
                validationError.details = errorBody.details; // Aggiungo i dettagli Zod dell'errore
...
export async function getOffers({ search, category } = {}) {
    ...
    // 1. Eseguo la chiamata e 3. gestisco la risposta (handleResponse si occupa dell'interpretazione degli errori)
